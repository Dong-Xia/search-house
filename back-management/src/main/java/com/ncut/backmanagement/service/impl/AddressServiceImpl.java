...
import com.ncut.backmanagement.common.BaiduMapLocation;
import com.ncut.backmanagement.common.ServiceMultiResult;
import com.ncut.backmanagement.common.ServiceResult;
import com.ncut.backmanagement.common.VO.SubwayDTO;
import com.ncut.backmanagement.common.VO.SubwayStationDTO;
import com.ncut.backmanagement.common.VO.SupportAddressDTO;
import com.ncut.backmanagement.dao.SubwayMapper;
import com.ncut.backmanagement.dao.SubwayStationMapper;
import com.ncut.backmanagement.dao.SupportAddressMapper;
import com.ncut.backmanagement.domain.Subway;
import com.ncut.backmanagement.domain.SubwayStation;
import com.ncut.backmanagement.domain.SupportAddress;
...
public class AddressServiceImpl implements IAddressService {
    ...
    @Autowired
    private SubwayMapper subwayRepository;

    @Autowired
    private SubwayStationMapper subwayStationRepository;
    ...
    public ServiceMultiResult<SupportAddressDTO> findAllCities() {
        ...
        List<SupportAddress> addresses = supportAddressRepository.selectByMap(map);
        List<SupportAddressDTO> addressDTOS = new ArrayList<>();
        for (SupportAddress supportAddress : addresses) {
            SupportAddressDTO target = new ModelMapper().map(supportAddress, SupportAddressDTO.class);
    ...
    public Map<SupportAddress.Level, SupportAddressDTO> findCityAndRegion(String cityEnName, String regionEnName) {
        ...
        SupportAddress supportAddress1 = new SupportAddress();
        supportAddress1.setLevel(SupportAddress.Level.CITY.getValue());
        supportAddress1.setEnName(cityEnName);
        SupportAddress city = supportAddressRepository.selectOne(supportAddress1);
        SupportAddress supportAddress = new SupportAddress();
        supportAddress.setEnName(city.getEnName());
        supportAddress.setBelongTo(regionEnName);
        SupportAddress region = supportAddressRepository.selectOne(supportAddress);

        result.put(SupportAddress.Level.CITY, new ModelMapper().map(city, SupportAddressDTO.class));
        result.put(SupportAddress.Level.REGION, new ModelMapper().map(region, SupportAddressDTO.class));
        return result;
    }
    ...
    public ServiceMultiResult<SupportAddressDTO> findAllRegionsByCityName(String cityName) {
        ...
        Map<String, Object> map = new HashMap<>();
        map.put("level", SupportAddress.Level.CITY.getValue());
        map.put("belong_to", cityName);
        List<SupportAddress> regions = supportAddressRepository.selectByMap(map);
        for (SupportAddress region : regions) {
            result.add(new ModelMapper().map(region, SupportAddressDTO.class));
        }
    ...
    public List<SubwayDTO> findAllSubwayByCity(String cityEnName) {
        ...
        Map<String, Object> map = new HashMap<>();
        map.put("city_en_name", cityEnName);
        List<Subway> subways = subwayRepository.selectByMap(map);
        if (subways.isEmpty()) {
            return result;
        }

        subways.forEach(subway -> result.add(new ModelMapper().map(subway, SubwayDTO.class)));
        return result;
    }
    ...
    public List<SubwayStationDTO> findAllStationBySubway(Long subwayId) {
        ...
        Map<String, Object> map = new HashMap<>();
        map.put("subway_id", subwayId);
        List<SubwayStation> stations = subwayStationRepository.selectByMap(map);
        if (stations.isEmpty()) {
            return result;
        }

        stations.forEach(station -> result.add(new ModelMapper().map(station, SubwayStationDTO.class)));
        return result;
    }
    ...
    public ServiceResult<SubwayDTO> findSubway(Long subwayId) {
        ...
        Subway subway = subwayRepository.selectById(subwayId);
        if (subway == null) {
            return ServiceResult.notFound();
        }
        return ServiceResult.of(new ModelMapper().map(subway, SubwayDTO.class));
    }
    ...
    public ServiceResult<SubwayStationDTO> findSubwayStation(Long stationId) {
        ...
        SubwayStation station = subwayStationRepository.selectById(stationId);
        if (station == null) {
            return ServiceResult.notFound();
        }
        return ServiceResult.of(new ModelMapper().map(station, SubwayStationDTO.class));
    }
    ...
    public ServiceResult<SupportAddressDTO> findCity(String cityEnName) {
        ...
        supportAddress1.setLevel(SupportAddress.Level.CITY.getValue());
        supportAddress1.setEnName(cityEnName);
        SupportAddress supportAddress = supportAddressRepository.selectOne(supportAddress1);
        if (supportAddress == null) {
            return ServiceResult.notFound();
        }

        SupportAddressDTO addressDTO = new ModelMapper().map(supportAddress, SupportAddressDTO.class);
